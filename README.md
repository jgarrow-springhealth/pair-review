...
- **Rendered mode** fetches the file's current content and renders it with the same markdown-it + DOMPurify pipeline already used for comments, so safe rendering behavior (sanitization, GitHub-supported inline HTML subset, and checked/unchecked task-list checkboxes) is identical to the rest of the app. Inline code uses a theme-aware accent tint, border, and monospace treatment so it remains visually distinct from surrounding prose in both light and dark themes. Rendered mode shows the whole document, not just the diffed hunks — useful for judging a documentation change in context.
...
- **Granular targets in lists and tables.** Lists and tables also expose their parts as comment and chat targets: an individual list item, a nested list item, a table row, a header cell or a single data cell — alongside (never instead of) the whole-list/whole-table target. Hovering or keyboard-focusing shows one leading chat/**+** pair for the innermost thing you are pointing at, and the target it applies to is outlined and named ("Table cell, line 71, column 2"). A row's controls sit in a slim presentational gutter at the start of the row, since controls cannot legally live between a row's cells.
  ...
  - Each target with feedback shows a small comment-count badge. The full card also stays beside the exact content: list-item feedback sits immediately after that item's text and before any nested list; row and cell feedback sits in a valid companion row immediately after the source row. Line-based AI findings—and the accepted comments that replace them—use the same nearby list-item or table-row placement when the line identifies one unambiguously; they never guess a table cell from a row-wide line number. This preserves table/list semantics without collecting nested feedback far below the line it discusses.
...
- **One review, two views of it.** Diff and Rendered are two surfaces over the same comments and AI findings, and they stay in step: active line-level AI findings appear beside their rendered source block; adopting one replaces it with the saved AI-origin comment instead of making it disappear; and a comment created, edited or dismissed in either view updates the other immediately and stays synchronized with the AI/Review panel. Clicking a user comment or AI finding in that panel jumps directly to its visible Rendered content and briefly outlines the annotated element; an adopted AI finding follows the saved child comment that replaced it rather than falling back to the old Diff suggestion. Navigation does not target hidden copies or wait for hidden hunks to render first. Feedback that cannot be represented honestly against current-file Markdown (such as a LEFT-side comment) switches the file to Diff and navigates there instead. Comments are counted once, no matter how many surfaces are currently showing them, so the toolbar count, "N comments will be submitted", Clear All and the Request-changes check always reflect the real amount of captured feedback.
